...
- Imports detached text-file copies either through the browser's file chooser or from a path on the Studio host, keeping the host-path option available in embedded and remote browser views; imported copies remain detached until saved or opened as file-backed documents
